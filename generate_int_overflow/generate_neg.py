...
def main(args):
    ...
    generators = [gen_cond_example_higher, gen_while_example_higher, gen_for_example_higher,
                gen_fv_cond_example_higher, gen_fv_while_example_higher, gen_fv_for_example_higher,
                gen_cond_example_lower, gen_while_example_lower, gen_for_example_lower,
                gen_fv_cond_example_lower, gen_fv_while_example_lower, gen_fv_for_example_lower
                ]
    if linear_only:
...
def gen_cond_example_lower(include_cond_bufwrite=True):
    ...
    """
    is_neg_add_neg = random.choice([True, False])
    anon_vars = _get_anon_vars()
    overflow_var, int_var, thresh_var = anon_vars[:3]
    dummy_vars = anon_vars[3:]
    thresh = random.randrange(MINIMUM_INT, MAXIMUM_INT)
    int_init = random.randrange(MINIMUM_INT, 0)
    true_int = random.randrange(MINIMUM_INT, 0) if is_neg_add_neg \
        else random.randrange(0, MAXIMUM_INT)
    false_int = random.randrange(MINIMUM_INT, 0) if is_neg_add_neg \
        else random.randrange(0, MAXIMUM_INT)
    char = _get_char()
    substitutions = {
        'int_var': int_var,
    ...
    main_lines = templates.COND_MAIN_LINES_LOWER
    cond = int_init < thresh

    if is_neg_add_neg:
        safe = int_init + true_int >MINIMUM_INT if cond \
            else int_init + false_int >MINIMUM_INT
    else:
        safe = int_init - true_int >MINIMUM_INT if cond \
            else int_init - false_int >MINIMUM_INT
    ...
    return _assemble_general_example(dec_init_pairs, main_lines, dummy_vars,
                                     safe, substitutions,
                                     include_cond_bufwrite, is_neg_add_neg)

def gen_cond_example_higher(include_cond_bufwrite=True):
    """Generate conditional example
...
def gen_while_example_lower(include_cond_bufwrite=True):
    ...
    """
    is_neg_add_neg = random.choice([True, False])
    enter_main_lines_add=random.choice([True,False])
    anon_vars = _get_anon_vars()
    overflow_var, int_var, count_var = anon_vars[:3]
    dummy_vars = anon_vars[3:]
    int_init = random.randrange(MINIMUM_INT, 0)
    overflow_int=random.randrange(MINIMUM_INT,MAXIMUM_INT)
    ITERATION=random.randrange(800000,4000000)
    if is_neg_add_neg:
        """进入while_minus块"""
        if overflow_int<=0:
            count_int=random.randrange(0,math.floor(( overflow_int-MINIMUM_INT)/ITERATION))
        else:
            count_int=random.randrange(math.ceil( overflow_int/ITERATION),math.floor(( overflow_int-MINIMUM_INT)/ITERATION))
        re_overflow = overflow_int-count_int*ITERATION
    else:
        """进入while_add块"""
        if( overflow_int>=0):
            count_int=random.randrange(0,math.floor((MAXIMUM_INT- overflow_int)/ITERATION))
        else:
            count_int=random.randrange(math.ceil((0- overflow_int)/ITERATION),math.floor((MAXIMUM_INT- overflow_int)/ITERATION))
        re_overflow =  overflow_int + count_int * ITERATION
    ...
    main_lines = templates.WHILE_MAIN_LINES_MINUS if is_neg_add_neg \
        else templates.WHILE_MAIN_LINES_ADD

    safe = int_init + re_overflow > MINIMUM_INT if is_neg_add_neg \
        else int_init-re_overflow>MINIMUM_INT
    dec_init_pairs = templates.WHILE_DEC_INIT_PAIRS_LOWER

    return _assemble_general_example(dec_init_pairs, main_lines, dummy_vars,
                                     safe, substitutions,
                                     include_cond_bufwrite,is_neg_add_neg)

def gen_while_example_higher(include_cond_bufwrite=True):
    """Generate while-loop example
    ...
    overflow_var, int_var, count_var = anon_vars[:3]
    dummy_vars = anon_vars[3:]
    int_init = random.randrange(MINIMUM_INT, -483648)
    int_count = math.floor(int_init / 1000000)
    count_int = random.randrange(-2147, 0)
    char = _get_char()
    substitutions = {
        'overflow_var': overflow_var,
        'int_var': int_var,
    ...
    main_lines = templates.WHILE_MAIN_LINES_ADD_HIGHER if is_pos_add_pos \
        else templates.WHILE_MAIN_LINES_MINUS_HIGHER
...
def gen_for_example_lower(include_cond_bufwrite=True):
    """Generate while-loop example
    ...
    main_lines = templates.FOR_MAIN_LINES_MINUS if is_neg_add_neg \
        else templates.FOR_MAIN_LINES_ADD

    safe = int_init + re_overflow > MINIMUM_INT if is_neg_add_neg \
        else int_init-re_overflow>MINIMUM_INT
    dec_init_pairs = templates.FOR_DEC_INIT_PAIRS_LOWER

    return _assemble_general_example(dec_init_pairs, main_lines, dummy_vars,
                                     safe, substitutions,
                                     include_cond_bufwrite,is_neg_add_neg)

def gen_for_example_higher(include_cond_bufwrite=True):
    ...
    main_lines = templates.FOR_MAIN_LINES_ADD_HIGHER if is_neg_add_neg \
        else templates.FOR_MAIN_LINES_MINUS_HIGHER
    safe = -(int_count + count_int) < 2147
...
def gen_fv_cond_example_lower(include_cond_bufwrite=True):
    """Generate conditional example
    ...
    thresh_int = random.randrange(MINIMUM_INT,0) if is_neg_add_neg \
        else random.randrange(0,MAXIMUM_INT)
    int_init = random.randrange(MINIMUM_INT, 0)
    false_int = random.randrange(MINIMUM_INT, 0) if is_neg_add_neg \
        else random.randrange(0, MAXIMUM_INT)
    ...
    main_lines = templates.COND_FV_MAIN_LINES_ADD_LOWER if is_neg_add_neg \
        else templates.COND_FV_MAIN_LINES_MINUS_LOWER
    safe = min(int_init + false_int, int_init + thresh_int-1) > MINIMUM_INT if is_neg_add_neg \
        else min(int_init - false_int, int_init - thresh_int+1) > MINIMUM_INT
...
def gen_fv_cond_example_higher(include_cond_bufwrite=True):
    ...
    main_lines = templates.COND_FV_MAIN_LINES_ADD_HIGHER if is_neg_add_neg \
        else templates.COND_FV_MAIN_LINES_MINUS_HIGHER
    safe = min(int_init+false_int, int_init+thresh_int) > MINIMUM_INT if is_neg_add_neg \
        else min(int_init-false_int, int_init-thresh_int) > MINIMUM_INT
    dec_init_pairs = templates.COND_FV_DEC_INIT_PAIRS
...
def gen_fv_while_example_lower(include_cond_bufwrite=True):
    """Generate while-loop example
    ...
    is_neg_add_neg = random.choice([True, False])
    anon_vars = _get_anon_vars()
    overflow_var, int_var, count_var,thresh_var,free_var = anon_vars[:5]
    ...
    if is_neg_add_neg:
        """进入while_minus块"""
        if overflow_int<=0:
            count_int_max=math.floor(( overflow_int-MINIMUM_INT)/ITERATION)
            count_int_min=0
        else:
            count_int_max = math.floor(( overflow_int-MINIMUM_INT)/ITERATION)
            count_int_min = math.ceil( overflow_int/ITERATION)
    else:
        """进入while_add块"""
        if( overflow_int>=0):
            count_int_max = math.floor((MAXIMUM_INT- overflow_int)/ITERATION)
            count_int_min = 0
        else:
            count_int_max = math.floor((MAXIMUM_INT- overflow_int)/ITERATION)
            count_int_min = math.ceil((0- overflow_int)/ITERATION)

    thresh_int=random.randrange(count_int_min,count_int_max)
    false_int=random.randrange(count_int_min,count_int_max)
    ...
    main_lines = templates.WHILE_FV_MAIN_LINES_MINUS_LOWER if is_neg_add_neg \
        else templates.WHILE_FV_MAIN_LINES_ADD_LOWER

    re_overflow= overflow_int - max(thresh_int - 1, false_int) * ITERATION if is_neg_add_neg \
        else overflow_int + max(thresh_int - 1, false_int)*ITERATION
    safe = int_init + re_overflow > MINIMUM_INT if is_neg_add_neg \
        else int_init-re_overflow>MINIMUM_INT
    dec_init_pairs = templates.WHILE_FV_DEC_INIT_PAIRS_LOWER

    return _assemble_general_example(dec_init_pairs, main_lines, dummy_vars,
                                     safe, substitutions,
                                     include_cond_bufwrite,is_neg_add_neg)
def gen_fv_while_example_higher(include_cond_bufwrite=True):
    ...
    main_lines = templates.WHILE_FV_MAIN_LINES_ADD_HIGHER if is_neg_add_neg \
        else templates.WHILE_FV_MAIN_LINES_MINUS_HIGHER
    safe = min(int_init+false_int, int_init+thresh_int) > MINIMUM_INT if is_neg_add_neg \
        else min(int_init-false_int, int_init-thresh_int) > MINIMUM_INT
...
def gen_fv_for_example_lower(include_cond_bufwrite=True):
    """Generate for-loop example
    ...
    main_lines = templates.FOR_FV_MAIN_LINES_MINUS_LOWER if is_neg_add_neg \
        else templates.FOR_FV_MAIN_LINES_ADD_LOWER

    re_overflow= overflow_int - max(thresh_int - 1, false_int) * ITERATION if is_neg_add_neg \
        else overflow_int + max(thresh_int - 1, false_int)*ITERATION
    safe = int_init + re_overflow > MINIMUM_INT if is_neg_add_neg \
        else int_init-re_overflow>MINIMUM_INT
    dec_init_pairs = templates.FOR_FV_DEC_INIT_PAIRS_LOWER

    return _assemble_general_example(dec_init_pairs, main_lines, dummy_vars,
                                     safe, substitutions,
                                     include_cond_bufwrite,is_neg_add_neg)
def gen_fv_for_example_higher(include_cond_bufwrite=True):
    ...
    main_lines = templates.FOR_FV_MAIN_LINES_ADD_HIGHER if is_neg_add_neg \
        else templates.FOR_FV_MAIN_LINES_MINUS_HIGHER
    safe = min(int_init+false_int, int_init+thresh_int) > MINIMUM_INT if is_neg_add_neg \
        else min(int_init-false_int, int_init-thresh_int) > MINIMUM_INT
...
def _test(verbose=False):
    ...
    for gen in [gen_cond_example_higher, gen_while_example_higher, gen_for_example_higher,
                gen_fv_cond_example_higher, gen_fv_while_example_higher, gen_fv_for_example_higher,
                gen_cond_example_lower, gen_while_example_lower, gen_for_example_lower,
                gen_fv_cond_example_lower, gen_fv_while_example_lower, gen_fv_for_example_lower
                ]:
